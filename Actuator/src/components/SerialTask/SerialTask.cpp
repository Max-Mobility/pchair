    ...
    if (xQueueReceive(uart0_queue, (void *)&event,
      ...
      switch (event.type) {
        ...
        /*We'd better handler data event fast, there would be much more data
           ...
           event, the queue might be full. in this example, we don't process
           data in event, but read data outside.*/
      case UART_DATA:
      ...
      case UART_FIFO_OVF:
        ...
        // for your application. We can read data out out the buffer, or directly
        // flush the rx buffer.
      ...
      case UART_BUFFER_FULL:
        ...
        // size We can read data out out the buffer, or directly flush the rx
        // buffer.
        uart_flush(EX_UART_NUM);
        break;
...
void taskFunction(void *pvParameter) {
  ...
  uart_driver_install(UART_NUM_1, BUF_SIZE * 2, BUF_SIZE * 2, 10, &uart1_queue,
                      0);
  uart_driver_install(UART_NUM_2, BUF_SIZE * 2, BUF_SIZE * 2, 10, &uart2_queue,
                      0);
  uart_set_pin(UART_NUM_1, GPIO_NUM_15, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE,
               UART_PIN_NO_CHANGE);
  uart_set_pin(UART_NUM_2, GPIO_NUM_17, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE,
               UART_PIN_NO_CHANGE);
...
void state_State_1_execute(void) {
  ...
  if (!__change_state__) {
    ...
    //	  DisplayTask::debugDisplay.addLog(str);
    //      leftSpeed[0]++;
    //      rightSpeed[0]++;
    //      memset(data, 0, BUF_SIZE);
    //      int len = uart_read_bytes(EX_UART_NUM, data, BUF_SIZE, 0);//100 /
    //      portTICK_RATE_MS); if (len > 0) {
...
void state_State_1_setState(void) { stateLevel_0 = state_State_1; }
