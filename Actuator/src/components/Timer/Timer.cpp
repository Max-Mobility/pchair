...
//	controlPinSetup(actEnable);
	...
	periodic_timer_args_2HZ.name ="periodic_2HZ" ;            /* name is optional, but may help identify the timer when debugging */


    esp_timer_handle_t periodic_timer_2HZ;
    ESP_ERROR_CHECK(esp_timer_create(&periodic_timer_args_2HZ, &periodic_timer_2HZ));
    ...
    periodic_timer_args_7500HZ.name ="periodic_7500HZ" ;            /* name is optional, but may help identify the timer when debugging */


    esp_timer_handle_t periodic_timer_7500HZ;
    ESP_ERROR_CHECK(esp_timer_create(&periodic_timer_args_7500HZ, &periodic_timer_7500HZ));
       /* The timer has been created but is not running yet */


    /* Start the timers */
    ESP_ERROR_CHECK(esp_timer_start_periodic(periodic_timer_2HZ, 2000000/25));
    ESP_ERROR_CHECK(esp_timer_start_periodic(periodic_timer_7500HZ, 1000000/7500));
...
//printf("this is the 2Hz timer\n");
	Actuator::updatePosition();  // calculate position by compare high and low counter.
