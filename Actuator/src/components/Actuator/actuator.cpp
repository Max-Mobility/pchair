...
ACT actElevation(act4ControlA, act4ControlB, pwmPin4, 90, 10,
                 ActElevationOutLimit);
...
bool stateChanged =
    false; // when this is true, save actuator position in tilt or stand mode.
bool system_mode_changed =
    true; // when this is true, send new system mode to the App.
std::string str;
  ...
  while (1) {
    if (I2C::joystickError) {
      ...
      case Actuator_recline: // only move recline actuator
        ...
        break;
      case Actuator_legrest: // only move legrest when legrest high are higher
                             // than 50mm
        switch (actionInput) {
        case nonMoving:
          actuatorStop();
        ...
        case movingRight:
          actLegrest.moveOut();
          //								if
          //(LegRestHeightToGround>75) 									actLegrest.moveOut(); 								else
          //									actLegrest.stop();
          break;
        }
        break;
      case Actuator_tilt: // change seat angle by moving tilt actuator or
                          // elevation actuator or both
        updateActPositionLimit();
        switch (actionInput) {
        case nonMoving:
          actuatorStop();
        ...
        break;
      case Actuator_elevation: // move elevation actuator and tilt actuator to
                               // mantain same seat angle
        if (stateChanged) {
          SavedSeatAngleToGround = SeatAngleToGround;
          //							printf("changed to elevation
          //state.\n");
          if (Actuator::actElevation.position < GoKartHeight - 3) {
      ...
      case Actuator_stand:
        if (stateChanged) {
          if (SeatAngleToGround > -10) {
            ...
            SavedSeatAngleToGround = SeatAngleToGround;
          } else // not normal seating position, so set to default seating
                 // position
          {
...
void checkButton() {
  ...
  if (buttonCounter_up == 2) // up button pressed to change mode from drive to
                             // act or from act to drive mode
  {
    if (systemMode == DriveMode) {
      systemMode = Actuator_recline;
    } else {
      if (systemMode != DriveMode && systemMode != ErrorMode &&
  ...
  //		printf("system mode  %d, low_counter %d, up_counter
...
action_input getActionInput(action_input act) {
  ...
  if ((dirCounter_up % 10 == 2) &&
      (BLE::actMovDir == BLE::Actuator_moving_dir::AMD_stop))
    _actionInput = movingUp;
  if ((dirCounter_down % 10 == 2) &&
      (BLE::actMovDir == BLE::Actuator_moving_dir::AMD_stop))
    _actionInput = movingDown;
  if ((dirCounter_left >= 2) ||
      (BLE::actMovDir == BLE::Actuator_moving_dir::AMD_left))
    _actionInput = movingLeft;
  if ((dirCounter_right >= 2) ||
      (BLE::actMovDir == BLE::Actuator_moving_dir::AMD_right))
...
void ACT::getPosition() {
  ...
  pos = (pos - ACT::pwm_low_limit) * 100.0 /
        (ACT::pwm_up_limit - ACT::pwm_low_limit);
  if (pos > 100)
    pos = 100;
  if (pos < 0)
    pos = 0;
...
ACT::ACT(gpio_num_t A, gpio_num_t B, gpio_num_t P, uint32_t up, uint32_t low,
         uint32_t out) {
  ACT::controlA = A;
  ACT::controlB = B;
  ACT::pwmInput = P;
...
void ACT::update() {
  if (ACT::positionUpdated) {
    if (!ACT::targetUpdated) // first time get pwm position and set it as
                             // target.
    {
...
void DriveToTarget(int32_t recline, int32_t tilt, int32_t leg,
                   int32_t elev) // target valid for 0-100, -1 for no action
{
  if (recline >= 0) {
...
void updateAngle() {
  ...
  LegRestJointAngleToGround =
      SeatAngleToGround + getLegRestJointAngle(Actuator::actLegrest.position);
  ...
  LegRestHeightToGround =
      SeatHeight -
      sin((LegRestJointAngleToGround)*3.14159 / 180.0) * LegRestJointHeight;
}
...
bool LegRestTouchGround() { return LegRestHeightToGround < 0; }
