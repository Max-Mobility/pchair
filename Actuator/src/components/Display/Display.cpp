...
uint8_t vram[DISPLAY_WIDTH * DISPLAY_WIDTH];
uint16_t myPalette[256] = {
    0,     8,     23,    31,    256,   264,   279,   287,   512,   520,   535,
    543,   768,   776,   791,   799,   1248,  1256,  1271,  1279,  1504,  1512,
    1527,  1535,  1760,  1768,  1783,  1791,  2016,  2024,  2039,  2047,  8192,
    8200,  8215,  8223,  8448,  8456,  8471,  8479,  8704,  8712,  8727,  8735,
    8960,  8968,  8983,  8991,  9440,  9448,  9463,  9471,  9696,  9704,  9719,
    9727,  9952,  9960,  9975,  9983,  10208, 10216, 10231, 10239, 16384, 16392,
    16407, 16415, 16640, 16648, 16663, 16671, 16896, 16904, 16919, 16927, 17152,
    17160, 17175, 17183, 17632, 17640, 17655, 17663, 17888, 17896, 17911, 17919,
    18144, 18152, 18167, 18175, 18400, 18408, 18423, 18431, 24576, 24584, 24599,
    24607, 24832, 24840, 24855, 24863, 25088, 25096, 25111, 25119, 25344, 25352,
    25367, 25375, 25824, 25832, 25847, 25855, 26080, 26088, 26103, 26111, 26336,
    26344, 26359, 26367, 26592, 26600, 26615, 26623, 38912, 38920, 38935, 38943,
    39168, 39176, 39191, 39199, 39424, 39432, 39447, 39455, 39680, 39688, 39703,
    39711, 40160, 40168, 40183, 40191, 40416, 40424, 40439, 40447, 40672, 40680,
    40695, 40703, 40928, 40936, 40951, 40959, 47104, 47112, 47127, 47135, 47360,
    47368, 47383, 47391, 47616, 47624, 47639, 47647, 47872, 47880, 47895, 47903,
    48352, 48360, 48375, 48383, 48608, 48616, 48631, 48639, 48864, 48872, 48887,
    48895, 49120, 49128, 49143, 49151, 55296, 55304, 55319, 55327, 55552, 55560,
    55575, 55583, 55808, 55816, 55831, 55839, 56064, 56072, 56087, 56095, 56544,
    56552, 56567, 56575, 56800, 56808, 56823, 56831, 57056, 57064, 57079, 57087,
    57312, 57320, 57335, 57343, 63488, 63496, 63511, 63519, 63744, 63752, 63767,
    63775, 64000, 64008, 64023, 64031, 64256, 64264, 64279, 64287, 64736, 64744,
    64759, 64767, 64992, 65000, 65015, 65023, 65248, 65256, 65271, 65279, 65504,
    65512, 65527, 65535};
...
void Draw_5x8_char(char *_char_matrix, int x_start, int y_start,
  ...
  for (col = 0; col <= 4; col++) {
    for (row = 0; row <= 7; row++) {
      if ((row + y_start) >= 0 && (row + y_start) < DISPLAY_HEIGHT &&
          (col + x_start) >= 0 && (col + x_start) < DISPLAY_WIDTH) {
...
void Draw_5x8_string(char *str, unsigned char len, int x_start, int y_start,
  ...
  int i = 0;
  for (i = 0; i < len; i++)
    Draw_5x8_char((char *)char5x8_matrix[(int)str[i]], x_start + i * 6, y_start,
                  clr);
}
...
void Draw_8x12_string(char *str, unsigned char len, int x_start, int y_start,
  ...
  int i = 0;
  for (i = 0; i < len; i++) {
    Draw_8x12_char((char *)char8x12_matrix[(int)str[i]], x_start + i * 9,
                   y_start, clr);
  }
}
...
void plot4points(int cx, int cy, int x, int y, unsigned char clroutline,
                 unsigned char clrfill) {
  int row, col;
  for (row = cy - y; row <= (cy + y); row++) {
    for (col = cx - x; col <= (cx + x); col++) {
      if (row >= 0 && row < DISPLAY_HEIGHT && col >= 0 && col < DISPLAY_WIDTH)
        vram[row + col * DISPLAY_HEIGHT] = clrfill;
    }
  }
  if ((cy + y) >= 0 && (cy + y) < DISPLAY_HEIGHT && (cx + x) >= 0 &&
      (cx + x) < DISPLAY_WIDTH)
...
void plot8points(int cx, int cy, int x, int y, unsigned char clroutline,
                 unsigned char clrfill) {
  plot4points(cx, cy, x, y, clroutline, clrfill);
  plot4points(cx, cy, y, x, clroutline, clrfill);
}

void circle(int cx, int cy, int radius, unsigned char clroutline,
            unsigned char clrfill) {
  int error = -radius;
  int x = radius;
  int y = 0;
...
void draw_circle_inline(const uint8_t line) {
  clear_vram(0, 120, 24, 96);
  point_s pos = {
      .x = 12,
      .y = (uint16_t)((line - 1) * 12 + 6 + 120 + 24),
  };
  draw_circle(pos, 4, 255, 255);
}

void draw_line(const point_s start, const point_s end, const uint8_t color) {
  int _dummy;
  int xLeft = start.x, xRight = end.x, yTop = start.y, yBottom = end.y;
  ...
  for (col = xLeft; col <= xRight; col++) {
    if (row >= 0 && col >= 0 && row < DISPLAY_HEIGHT && col < DISPLAY_WIDTH) {
      if (steep)
        vram[col + row * DISPLAY_HEIGHT] = color;
      else
        vram[row + col * DISPLAY_HEIGHT] = color;
    }
    error = error - dy;
    if (error < 0) {
      row = row + ystep;
...
void ili9341_write_frame(const uint16_t x, const uint16_t y,
                         const uint16_t width, const uint16_t height,
                         const uint8_t *data);

void init_vram() {
  //	vram = (uint8_t *)heap_caps_malloc(320 * 240 , MALLOC_CAP_8BIT);
}

void clear_vram() {
  memset(vram, 0, DISPLAY_WIDTH * DISPLAY_WIDTH);
  // memset(vram, 0, DISPLAY_WIDTH * DISPLAY_HEIGHT);
}

void clear_vram(const uint16_t x, const uint16_t y, const uint16_t width,
                const uint16_t height) {
  int xs = MAX(0, x), xe = MIN(DISPLAY_WIDTH, xs + width), ys = MAX(0, y),
      // ye = MIN(DISPLAY_HEIGHT, ys + height),
      ye = MIN(DISPLAY_WIDTH, ys + height), len = ye - ys;
  for (int i = xs; i < xe; i++) {
    // memset( &vram[ ys + i * DISPLAY_HEIGHT ], 0, len );
    memset(&vram[ys + i * DISPLAY_WIDTH], 0, len);
  }
}

void display_vram() {
  ili9341_write_frame(0, 0, DISPLAY_WIDTH, DISPLAY_HEIGHT,
                      (const uint8_t *)vram);
}

void blit_vram(const uint16_t xs, const uint16_t ys, const uint16_t width,
               const uint16_t height) {
  int x, y;
  int i;
  uint16_t x1, y1;
  ...
  for (y = 0; y < height; y++) {
    ...
    while (READ_PERI_REG(SPI_CMD_REG(SPI_NUM)) & SPI_USR)
      ;
    GPIO.out_w1tc = dc;
    SET_PERI_REG_BITS(SPI_MOSI_DLEN_REG(SPI_NUM), SPI_USR_MOSI_DBITLEN, 7,
    ...
    while (READ_PERI_REG(SPI_CMD_REG(SPI_NUM)) & SPI_USR)
      ;
    GPIO.out_w1ts = dc;
    SET_PERI_REG_BITS(SPI_MOSI_DLEN_REG(SPI_NUM), SPI_USR_MOSI_DBITLEN, 31,
                      SPI_USR_MOSI_DBITLEN_S);
    ...
    x = 0;
    GPIO.out_w1ts = dc;
    SET_PERI_REG_BITS(SPI_MOSI_DLEN_REG(SPI_NUM), SPI_USR_MOSI_DBITLEN, 511,
                      SPI_USR_MOSI_DBITLEN_S);
    while (x < width) {
      for (i = 0; i < 16; i++) {
        x1 = myPalette[(unsigned char)(vram[y + x * DISPLAY_HEIGHT])];
        x++;
        y1 = myPalette[(unsigned char)(vram[y + x * DISPLAY_HEIGHT])];
        x++;
...
// ST7789V initial for V2 V3

static void ST7789V_INITIAL() {
  LCD_BKG_OFF();
  //------------------------------------Reset
  //Sequence-----------------------------------------//
...
void ili9341_write_frame(const uint16_t xs, const uint16_t ys,
                         const uint16_t width, const uint16_t height,
                         const uint8_t *data) {
  int x, y;
  int i;
  ...
  for (y = 0; y < height; y++) {
    ...
    while (x < width) {
      for (i = 0; i < 16; i++) {
        ...
        x1 = myPalette[(unsigned char)(data[y + x * height])];
        x++;
        y1 = myPalette[(unsigned char)(data[y + x * height])];
        x++;
        temp[i] = U16x2toU32(x1, y1);
      }
