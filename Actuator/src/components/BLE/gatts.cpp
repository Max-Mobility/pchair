 ...
 *gatt_server_service_table demo. Client demo will enable
 *gatt_server_service_table's notify after connection. Then two devices will
 *exchange data.
 *
 ****************************************************************************/

#include "include/gatts.hpp"
...
#define SAMPLE_DEVICE_NAME "ESP32_GATTS"
#define SVC_INST_ID 0

/* The max length of characteristic value. When the gatt client write or prepare
 * write, the data length must be less than GATTS_DEMO_CHAR_VAL_LEN_MAX.
 */
...
static uint8_t raw_adv_data[] = {
    ...
    0x03, 0x03, 0xFF, 0x12,
    /* device name */
    0x0f, 0x09, 'E', 'S', 'P', '3', '2', '_', 'G', 'A', 'T', 'T', 'S'};
...
/* Service */
static const uint16_t GATTS_SERVICE_UUID_TEST = 0x12FF;
...
static void gatts_profile_event_handler(esp_gatts_cb_event_t event,
  ...
  case ESP_GATTS_WRITE_EVT:
    ...
    if (param->write.handle ==
        pChair_handle_table[IDX_CHAR_VAL_A]) // write value to charA
    {
  ...
  case ESP_GATTS_CREAT_ATTR_TAB_EVT: {
    if (param->add_attr_tab.status != ESP_GATT_OK) {
      ...
      memcpy(pChair_handle_table, param->add_attr_tab.handles,
             sizeof(pChair_handle_table));
  ...
  do {
    ...
    for (idx = 0; idx < PROFILE_NUM; idx++) {
      ...
      if (gatts_if == ESP_GATT_IF_NONE ||
          gatts_if == Pchair_profile_tab[idx].gatts_if) {
  ...
  while (1) {
    if (BLE_ready) {
      if (Actuator::system_mode_changed) {
        ...
        ret = esp_ble_gatts_set_attr_value(pChair_handle_table[IDX_CHAR_VAL_A],
                                           2, data_send);
        ret = esp_ble_gatts_send_indicate(
            Pchair_profile_tab[PROFILE_APP_IDX].gatts_if, con_id,
            pChair_handle_table[IDX_CHAR_VAL_A], 2, data_send, false);
        Actuator::system_mode_changed = false;
        // printf("error:%d\n", (int)ret);
      } else {
        if ((Actuator::systemMode == Actuator::system_modes::DriveMode) ||
            (Actuator::systemMode ==
             Actuator::system_modes::PhoneControlMode)) // drive mode, send
                                                        // speed
        {
          data_send[0] = CMD_DRIVE_SPEED;
          data_send[1] =
              abs((int8_t)SerialTask::leftSpeed[1]); // SEND left motor speed as
                                                     // driving speed
          ret = esp_ble_gatts_set_attr_value(
              pChair_handle_table[IDX_CHAR_VAL_A], 2, data_send);
          ret = esp_ble_gatts_send_indicate(
              Pchair_profile_tab[PROFILE_APP_IDX].gatts_if, con_id,
              pChair_handle_table[IDX_CHAR_VAL_A], 2, data_send, false);
        }
