	...
	void taskFunction ( void *pvParameter )
	{
		while(1)
				...
				{ 	data_wr[0]=0x00;
					...
					if (err_check!=0 || (data_rd[2]==255&&data_rd[3]==255&&data_rd[4]==255))
					{
...
//						printf("X=%d; ", joystickX);
//						printf("Y=%d; ", joystickY);
//						if (joystickButtonHold_low)
//						{
//							printf("Button_Low pressed; ");
//						}
//						else
//						{
//							printf("Button_Low released; ");
//						}
//
//						if (joystickButtonHold_up)
//						{
//							printf("Button_Up pressed; ");
//						}
//						else
//						{
//							printf("Button_Up released; ");
//						}
//
//
//						printf("\n");
...
//		printf("start joystick.\n");
	...
	esp_err_t i2c_master_read_slave(i2c_port_t i2c_num, uint8_t *data_rd, size_t size)
	{
		...
		i2c_master_write_byte(cmd, (ESP_SLAVE_READ_ADDR << 1) | READ_BIT, ACK_CHECK_EN);
		if (size > 1) {
	...
	esp_err_t i2c_master_write_slave(i2c_port_t i2c_num, uint8_t *data_wr, size_t size)
	{
		i2c_cmd_handle_t cmd = i2c_cmd_link_create();
		i2c_master_start(cmd);
		i2c_master_write_byte(cmd, (ESP_SLAVE_WRITE_ADDR << 1) | WRITE_BIT, ACK_CHECK_EN);
