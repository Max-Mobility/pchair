...
int graphHeight = DISPLAY_HEIGHT * 1 / 2;
  ...
  for (int i = 1; i < MAX_PLOT_DATA_LEN; i++) {
    draw_line(
        {(uint16_t)(left + ((i - 1) * right) / MAX_PLOT_DATA_LEN),
         (uint16_t)(bottom -
                    ((plot->data[i - 1] - plot->min) * (bottom - top)) /
                        plot->range)},
        {(uint16_t)(left + (i * right) / MAX_PLOT_DATA_LEN),
         (uint16_t)(bottom - ((plot->data[i] - plot->min) * (bottom - top)) /
                                 plot->range)},
        plot->color);
  }
}
...
void GraphDisplay::clearPlots(void) { _numPlots = 0; }
...
void TextDisplay::init(void) { clearLogs(); }
...
void state_Update_Text_setState(void) { stateLevel_0 = state_Update_Text; }
...
void state_Update_Graph_setState(void) { stateLevel_0 = state_Update_Graph; }
...
void state_Wait_For_Data_execute(void) {
  ...
  if (!__change_state__) {
    ...
    static const std::string commandDelim =
        "+++"; // should start with this for command
    static const std::string shiftPlotCommand =
        "SHIFT PLOT:"; // followed by log name
    static const std::string shiftPlotsCommand = "SHIFT PLOTS";
    static const std::string removePlotCommand =
        "REMOVE PLOT:"; // followed by log name
      ...
      while (std::getline(ss, line, '\n')) {
        ...
        if ((pos = line.find(commandDelim)) != std::string::npos) {
          ...
          if (command == clearLogsCommand) {
            ...
            // make sure we transition to the next state
            hasNewPlotData = true;
          } else if ((pos = line.find(removePlotCommand)) !=
                     std::string::npos) {
            plotName =
                line.substr(pos + removePlotCommand.length(), line.length());
...
void state_Wait_For_Data_setState(void) { stateLevel_0 = state_Wait_For_Data; }
