...
/* FreeRTOS event group to signal when we are connected to WiFi and ready to
 * start UDP test*/
...
void wifi_init_sta() {
  ...
  wifi_config_t wifi_config = {
      .sta = {.ssid = EXAMPLE_DEFAULT_SSID, .password = EXAMPLE_DEFAULT_PWD},
  };
...
esp_err_t create_udp_client() {
  ...
  ESP_LOGI(TAG, "connecting to %s:%d", EXAMPLE_DEFAULT_SERVER_IP,
...
void close_socket() { close(mysocket); }
